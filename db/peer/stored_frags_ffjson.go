...
const (
	...
	ffjtStoredFragsFrags
)

var ffjKeyStoredFragsFrags = []byte("frags")
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				...
				case 'f':
					...
					if bytes.Equal(ffjKeyStoredFragsFrags, kn) {
						currentKey = ffjtStoredFragsFrags
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.EqualFoldRight(ffjKeyStoredFragsFrags, kn) {
					currentKey = ffjtStoredFragsFrags
					state = fflib.FFParse_want_colon
					goto mainparse
				}
			...
			for {
...

				{
					if tok == fflib.FFTok_null {
						...
						err = tmpJFrags.UnmarshalJSONFFLexer(fs, fflib.FFParse_want_key)
						if err != nil {
							return err
						}
					}
				...
				j.Frags = append(j.Frags, tmpJFrags)
